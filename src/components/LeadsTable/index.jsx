...
export default function LeadsTable({ filterdLeads = [], heads = [] }) {
    ...
    const handleDownloadExcel = async () => {
        try {
            setLoading(true)
            const res = await api.post(`files/download/leads`, filteredData, null, { responseType: "blob" });
    ...
    return (
        <>
            <button onClick={handleDownloadExcel} className={styles.excel}>
                {loading ? 'טוען...' : <> <GrDocumentExcel />הורדת הלידים כאקסל </>}
            </button>
