...
export default function Plans() {
  ...
  const star = (
    <svg width={13} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
  ...
  useEffect(() => {
    api
      .get("/plans")
      .then((res) => {
        // מיון המערך לפי המחיר
        const sortedPlans = res.sort((a, b) => a.price - b.price);
          ...
          {plansData?.map((plan, index) => (
            <div key={index}>
              <div className={styles.card}>
                {plan.name === "basic"
                  ? ((
                      <h2 className={`${styles.header} ${styles.grey}`}>
                        {plan.name}{" "}
                      </h2>
                    ),
                ...
                <ul className={styles.details}>
                  ...
                  {plan.num_leads_in_list > 0 ? (
                    <li>
                      {star}&nbsp; עד {plan.num_leads_in_list} קמפיינים
                    </li>
                  ) : (
                  ...
                  {plan.opening_msg_to_new_lids < 0 ? (
                    <li>
                      {star}&nbsp; עד {plan.opening_msg_to_new_lids} תווים
                      להודעת פתיחה
                    </li>
                  ) : (
                  ...
                  <li>
                    {star}&nbsp; עד {plan.msg_number} הודעות
                  </li>
                  {plan.data_transfer_crm ? (
                    <li>
                      {star}&nbsp; כולל העברת נתונים ל-CRM{" "}
                      {getIcon(plan.data_transfer_crm)}
                    </li>
                  ) : (
                    <li>
                      {star}&nbsp; ללא העברת נתונים ל-CRM{" "}
                      {getIcon(plan.data_transfer_crm)}
                    </li>
                  )}
                </ul>
                <button className={styles.buy} onClick={() => handlePlan(plan)}>
                  לרכישה
                </button>
              </div>
            </div>
